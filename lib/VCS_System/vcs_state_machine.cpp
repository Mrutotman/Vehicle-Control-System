...
#include "vcs_uart.h"
#include "vcs_pins.h"
#include "vcs_embutton.h" // For physical E-Stop check
...
void updateStateMachine(uint32_t faults) {
    ...
    switch (currentState) {
        ...
        case MANUAL_STATE:
            ...
            // AND the RPi must also be requesting Auto Mode.
            if (digitalRead(PIN_DMS_BUTTON) == LOW && getRPiCommandMode() == 1) {
        ...
        case FAULT_STATE:
            // In v1.3, reset only happens if Pi is re-linked and DMS is cleared
